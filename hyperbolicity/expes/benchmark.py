...
parser.add_argument('--dataset', type=str, required=True, help="Dataset name (e.g., 'wiki', 'airport', 'celegan', 'csphd', 'cora').")
parser.add_argument('--num_expe', type=int, default=100, help="Number of experiments to run.")
parser.add_argument('--methods', type=str, nargs='+', default=['hcc', 'gromov', 'treerep', 'layering'], 
...
if 'treerep' in methods:
    ...
    for _ in range(num_expe):
        ...
        distances_tr = np.array([[distances_tr[i][j] if j in distances_tr[i] else float('inf') 
...
if 'layering' in methods and dataset not in ['zeisel', 'cbmc', 'microbiote']:
    ...
    for j in indices:
        ...
        distances_lt = np.array([[distances_lt[i][j] if j in distances_lt[i] else float('inf') 
                                  for j in range(num_nodes)] for i in range(num_nodes)])
        elapsed_time.append(time.time() - start_time)
